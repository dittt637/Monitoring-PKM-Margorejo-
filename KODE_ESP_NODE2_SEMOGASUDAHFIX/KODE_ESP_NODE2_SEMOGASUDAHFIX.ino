...
const char* THINGSPEAK_WRITE_KEY_PENDUKUNG = "Y9WHRFN4ZDJXCTOR";
...
const uint8_t JUMLAH_SOIL_AKTIF = 0;
...
/*
  PERBAIKAN:
  WiFi tidak perlu dihentikan pada ESP32-S3.
  Fungsi hentikanWifiUntukSoil dan
  aktifkanWifiSetelahSoil telah dihapus.
*/
...
void bacaSensorKelembapan() {
  ...
  */
  ...
  for (
    ...
    if (i >= JUMLAH_SOIL_AKTIF) {
      /*
        Probe tidak terpasang. Pin tidak dibaca sama
        sekali agar tidak menghasilkan nilai mengambang.
      */
  ...
  for (
    ...
    if (soilValid[i]) {
      ...
      /*
        Jika persentase <= 1 persen,
        sensor dianggap tidak valid
        (kemungkinan tidak terhubung
        atau berada di udara terbuka).
      */

      if (
          persenSoil[i] <=
          BATAS_PERSEN_MINIMUM
      ) {
        soilValid[i] =
            false;
...
void kontrolValve(
  ...
  /*
    Minimal dua sensor harus valid.
  */
  ...
  if (valveTerbuka) {
    bool minimalDuaSensorBasah =
        jumlahBasah >=
        JUMLAH_BASAH_TUTUP;

    bool waktuMaksimumTercapai =
        sekarang -
        waktuValveMulai >=
        MAKSIMUM_WAKTU_VALVE;

    if (
        minimalDuaSensorBasah ||
        waktuMaksimumTercapai
    ) {
      setValve(false);
...
void kirimDataThingSpeak() {
  ...
  */
  ...
  /*
    field6: Suhu udara luar rata-rata (°C).
  */
  ...
  /*
    field7: Kelembapan udara luar rata-rata (%).
  */
...
// =====================================================
// KIRIM DATA THINGSPEAK - CHANNEL PENDUKUNG
// =====================================================
...
void kirimDataThingSpeakPendukung() {
  ...
  /*
    field1 dan field2: suhu dan kelembapan dari probe NPK.
    Dilewati jika pembacaan Modbus gagal.
  */
  ...
  if (dataNpk.valid) {
    ...
    body += "&field2=";
    body += String(dataNpk.kelembapan, 1);
  }

  /*
    field3 sampai field6: monitoring daya INA226.
      field3 = tegangan sisi baterai (SCC -> baterai)
      field4 = tegangan sisi load    (SCC -> load)
      field5 = arus sisi baterai
      field6 = arus sisi load
  */
  ...
  */
  ...
  /*
    field8: status aktuator sebagai bitmask.
    bit0 = kipas, bit1 = lampu,
    bit2 = valve, bit3 = valve timeout.
  */

  uint8_t statusAktuator =
      0;
